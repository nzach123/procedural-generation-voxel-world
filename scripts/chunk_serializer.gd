...
static func save_chunk(chunk: Chunk) -> void:
	...
	file.close()
	DebugLogger.debug("Saved chunk (%d, %d) - %d bytes" % [key.x, key.y, rle_data.size()], "ChunkSerializer")
...
static func load_chunk(coord: Vector2i) -> PackedByteArray:
	...
	DebugLogger.debug("Loaded chunk (%d, %d) - %d voxels" % [coord.x, coord.y, voxels.size()], "ChunkSerializer")
...
static func encode_rle(data: PackedByteArray) -> PackedByteArray:
	...
	for i in range(1, data.size()):
		...
		if value == current_value and count < VoxelConstants.MAX_RLE_RUN:
			count += 1
		else:
			result.append(count)
