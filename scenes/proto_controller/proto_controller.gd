...
var _default_fov: float = 75.0  ## Stored on ready
...
func _ready() -> void:
	...
	if $Head/Camera3D:
		_default_fov = $Head/Camera3D.fov
...
## Applies a temporary FOV kick (zoom in/out) for game feel.
## @param amount: Degrees to add to FOV (positive = widen/speed, negative = zoom/impact).
## @param duration: Total duration of the effect.
func apply_fov_kick(amount: float, duration: float) -> void:
	var cam = $Head/Camera3D
	if not cam:
		return

	# Cancel existing tween-kicks if any? For now, overlap is fine or simple overwrite.
	# Proper juice systems track tweens, but MVP: fire and forget.

	var tween = create_tween()
	# Kick out fast
	tween.tween_property(cam, "fov", _default_fov + amount, duration * 0.2).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	# Return slow
	tween.tween_property(cam, "fov", _default_fov, duration * 0.8).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
